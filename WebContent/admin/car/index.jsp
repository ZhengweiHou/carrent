...
function top(){
   	form3.action="<%=basePath%><%=dir %>/car/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/car/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/car/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/car/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/car/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/car/index.jsp?page="+page;
      form3.submit();
...
<BODY scroll=yes marginheight="0" marginwidth="0">
	   <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all border=1>
		  ...
		  <TR align="center" >
			...
			<TD class=gridViewHeader scope=col>车辆牌号</TD>
			<TD class=gridViewHeader scope=col>品牌型号</TD>
			<TD class=gridViewHeader scope=col>生产厂家</TD>
			<TD class=gridViewHeader scope=col>购买时间</TD>
			<TD class=gridViewHeader scope=col>价格</TD>
			<TD class=gridViewHeader scope=col>目前状态</TD>  
			<TD class=gridViewHeader scope=col>是否出租</TD>
			<TD class=gridViewHeader scope=col>修改</TD>
			<TD class=gridViewHeader scope=col>删除</TD>
		  </TR>
		<%
		cb.setEVERYPAGENUM(10);
			int cou = cb.getMessageCount("select count(*) from car");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from car order by id desc",10); //带进一个页数，并返回该页所要显示的信息 
		...
		%>
		  <TR  align="center" bgColor=#ffffff>
			...
			<TD  class=gridViewItem ><%=list3.get(9).toString() %></TD>
			<TD  class=gridViewItem ><a href="<%=basePath %>admin/car/add.jsp?method=upCAR&id=<%=list3.get(0).toString()%>">修改</a></TD>
			<TD  class=gridViewItem ><a href="<%=basePath %>ComAction.do?method=delCAR&id=<%=list3.get(0).toString()%>">删除</a></TD>
		  </TR>
		<%}}%>
		 </TBODY>
	   </TABLE>
