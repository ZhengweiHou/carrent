...
function top(){
   	form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/zl/index.jsp?page="+page;
      form3.submit();
...
<%
	...
	if(username==null){
		...
		//List list2=cb.getCom("select * from zj order by id desc",7);		
...
<BODY scroll=yes marginheight="0" marginwidth="0">
	   <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all border=1>
		  ...
		  <TR align="center" >
			...
			<TD class=gridViewHeader scope=col>押金（元）</TD>
			...
			<TD class=gridViewHeader scope=col>修改/查看</TD>
			<TD class=gridViewHeader scope=col>删除</TD>
		  </TR>
		<%cb.setEVERYPAGENUM(10);
			int cou = cb.getMessageCount("select count(*) from zj");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from zj order by id desc",8); //带进一个页数，并返回该页所要显示的信息 
		...
		%>
		  <TR  align="center" bgColor=#ffffff>
			...
			<TD  class=gridViewItem ><%=list3.get(7).toString() %></TD>
			<TD  class=gridViewItem ><%=list3.get(4).toString() %></TD>
			<TD  class=gridViewItem ><%=list3.get(5).toString() %></TD>
			<TD  class=gridViewItem >
			<%if(list3.get(6).toString().equals("否")){ %>
			<a href="<%=basePath %>ComAction.do?method=ghZL&id=<%=list3.get(0).toString()%>">确认状态完好 归还</a>
			<%}else{ %>
			<%=list3.get(6).toString() %>
			<%} %></TD>  
			<TD  class=gridViewItem ><a href="<%=basePath %>admin/zl/add.jsp?method=upZL&id=<%=list3.get(0).toString()%>">修改</a></TD>
			<TD  class=gridViewItem ><a href="<%=basePath %>ComAction.do?method=delZL&id=<%=list3.get(0).toString()%>">删除</a></TD>
		  </TR>
		<%}}%>
		 </TBODY>
	   </TABLE>
