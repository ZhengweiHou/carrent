...
function top(){
   	form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/zjg/index.jsp?page="+page;
      form3.submit();
...
<BODY scroll=yes marginheight="0" marginwidth="0">
	   <TABLE id="content" class=gridView id=ctl00_ContentPlaceHolder2_GridView1 style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all border=1>
		...
		<%cb.setEVERYPAGENUM(10);float f=0;
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from zj order by id desc",7); //带进一个页数，并返回该页所要显示的信息 
		...
		if(pagelist3.size()!=0){
			for(int i = 0; i<pagelist3.size(); i++){
				...
				f+=Float.parseFloat(list3.get(3).toString());
		%>
